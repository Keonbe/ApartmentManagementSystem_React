...
$stmt = $conn->prepare("UPDATE cctv_requests SET status = ? WHERE id = ?");
$stmt->bind_param("si", $status, $id);

if ($stmt->execute()) {
    log_activity($conn, $admin_id, 'service', "CCTV Request $status", "Request ID: $id");
    echo json_encode(["success" => true, "message" => "CCTV request $status"]);
} else {
